...
export default function PracticePage() {
  console.log('PracticePage: motion is', typeof motion);
  ...
  const handleSelectQuestion = useCallback(async (q) => {
    setOutput('ðŸ“¡ Loading full problem description...');
    setOutStatus('idle');
    setTestResults([]);
    setHasRun(false);
    setRuntimeMs(null);
    try {
      ...
      if (res.data.codeSnippets) {
        const slugMap = { python: 'python3', javascript: 'javascript', cpp: 'cpp', java: 'java', c: 'c' };
        const snippet = res.data.codeSnippets.find(s => s.langSlug === slugMap[language]);
        if (snippet) setCode(snippet.code);
      } else {
        setCode(getFallbackCode(language, res.data.titleSlug));
      }
      setOutput('Full description loaded. Start coding!');
      setOutStatus('success');
    } catch {
      setProblem(q); 
      setCode(getFallbackCode(language, q.titleSlug));
      setOutput('âš ï¸  Loaded without full description.');
      setOutStatus('error');
    }
    setHints([]);
    setShowPanel(false);
  ...
  useEffect(() => {
    const fetchRandom = async () => {
      try {
        ...
        setOutput('âš ï¸  Failed to load problems. Check backend connection.');
        setOutStatus('error');
      }
    };
    fetchRandom();
  ...
  // When language changes, load the right snippet for the CURRENT problem
  useEffect(() => {
    if (!problem) return;
    const slugMap = { python: 'python3', javascript: 'javascript', cpp: 'cpp', java: 'java', c: 'c' };
    
    const applyScaffold = async () => {
      // 1. Try to use built-in snippets first
      ...
      // 2. If no snippet, try to generate one via AI
      try {
  ...
  // Coach trigger: 3+ consecutive negative emotions
  useEffect(() => {
  ...
  // Focus recovery: check every 30s for inactivity >60s or engagement <30%
  useEffect(() => {
  ...
  const runCode = async () => {
    if (!problem) return;
    setIsRunning(true);
    setOutput('ðŸš€ Running your code against test cases...');
    setOutStatus('idle');
    setTestResults([]);
    try {
      ...
      addToast(r.data.success ? 'success' : 'error',
        ...
        r.data.success ? 'âœ… Code Executed' : 'âŒ Execution Failed');
    } catch {
      setOutput('âš ï¸  Execution failed. Is the backend running?');
      setOutStatus('error');
  ...
  const submitCode = async () => {
    if (!problem) return;
    if (!hasRun) {
      setOutput('âš ï¸  Please click Run first!');
      setOutStatus('error');
      return;
    }
    setIsRunning(true);
    setOutput('ðŸ“¡ Submitting and syncing results...');
    setOutStatus('idle');
    try {
      const r = await api.post('/api/run_code', {
        code, language, titleSlug: problem.titleSlug, id: problem.id,
      ...
      if (r.data.success) {
        ...
        addToast('success', `${problem.title} solved!`, 'ðŸŽ‰ Submitted');
        skipProblem();
      }
    } catch {
      setOutput('âš ï¸  Submission failed.');
      setOutStatus('error');
  ...
  const askMentor = async () => {
    ...
    try {
      ...
      setActiveBottomTab('hints'); // Auto-switch to hints tab
      addToast('hint', 'A new hint has been added to the AI panel.', 'ðŸ’¡ Hint Unlocked');
    } catch {
                  ...
                  <div className="relative">
                    ...
                    <div className="monaco-box">
                      <CodeEditor
                        ...
                        setCode={(val) => {
                          setCode(val);
                          lastTypedRef.current = Date.now(); // track typing activity
                        }}
                        onCpmChange={setCpm}
                        isDark={theme === 'dark'}
                        language={language}
      ...
      {showFocusPopup && (
        <FocusRecoveryPopup
          ...
          onComplete={() => {
            ...
            lastTypedRef.current = Date.now(); // reset idle clock
          }}
        />
      )}
